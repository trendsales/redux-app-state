...
const afterNavigate = (pages) => {
  ...
  // If the current page has meta date to it
  if (currentPage.meta) {
    const meta = currentPage.meta;
    // We get the page tag for the page
    const titleDom = document.querySelector('title');
    // and if it exists
    if (titleDom) {
      // We set it to the pages title
      titleDom.innerHTML = meta.title;
    }
    // We get the description tag for the page
    const descriptionDom = document.querySelector('meta[name="description"]');
    // and if it exists
    if (descriptionDom) {
      // We set it to the pages description
      descriptionDom.innerHTML = meta.description || '';
    }

    let canonicalDom = document.querySelector('link[rel="canonical"]');
    if (meta.desktopUrl) {
      if (!canonicalDom) {
        canonicalDom = document.createElement('link');
        canonicalDom.rel = 'canonical';
        document.head.append(canonicalDom);
      }
      canonicalDom.href = meta.desktopUrl || '';
    } else if (canonicalDom) {
      canonicalDom.parentElement.removeChild(canonicalDom);
    }
  }
};
