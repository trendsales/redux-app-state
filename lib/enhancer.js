...
export default ({
  ...
  const enhancer = (state = initState || {}, action) => {
    ...
    // navigated to.
      ...
      const page = commits[commits.length - 1];
    ...
    if (canGoBack) {
      historyResult.history.hasBack = canGoBack(commits, navigationState);
    }
    ...
    if (action.type === '@@history/NAVIGATE' || action.type === '@@history/TRAVEL') {
      ...
      if (trackPageView) {
        const isBack = action.type !== '@@history/NAVIGATE';
        trackPageView(currentPage.url, currentPage, commits, isBack);
      }
